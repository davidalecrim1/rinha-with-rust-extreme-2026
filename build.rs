...
//   - bbox_max.bin    (K x 14 x i16 = 28 KB)
//
...
fn dict_dim1() -> Vec<i16> {
    (0u32..=12)
        .map(|k| quantize((k as f32 / 12.0).clamp(0.0, 1.0)))
        .collect()
}
...
fn dict_dim8() -> Vec<i16> {
    (0u32..=20)
        .map(|k| quantize((k as f32 / 20.0).clamp(0.0, 1.0)))
        .collect()
}
...
fn quantized_search_vector(
    ...
    let i1 = find_dict_index(d1, quantize(v[1])) as usize;
    let i3 = find_dict_index(d3, quantize(v[3])) as usize;
    let i4 = find_dict_index(d4, quantize(v[4])) as usize;
    let i8 = find_dict_index(d8, quantize(v[8])) as usize;
    let i12 = find_dict_index(d12, quantize(v[12])) as usize;
    ...
    [
        ...
        quantize(v[7]),
        d8[i8],
        if v[9] > 0.5 { 8192 } else { 0 },
        if v[10] > 0.5 { 8192 } else { 0 },
        if v[11] > 0.5 { 8192 } else { 0 },
        d12[i12],
        quantize(v[13]),
    ]
}
...
fn kmeans(vectors: &[[f32; 14]]) -> (Vec<[f32; 14]>, Vec<u32>) {
    ...
    let mut centroids: Vec<[f32; 14]> = indices[..K].iter().map(|&i| vectors[i]).collect();
...
fn main() {
    ...
    std::fs::write(out_dir.join("dicts.rs"), &dicts).expect("failed to write dicts.rs");
